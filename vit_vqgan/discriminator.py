...
#     def __init__(self, image_channels=3, num_filters_last=64):
#         super().__init__()
#         layers = [
#             nn.Conv2d(image_channels, num_filters_last, 4, 2, 1, bias=True),
#             nn.LeakyReLU(0.2, inplace=True),

#             nn.Conv2d(num_filters_last, num_filters_last * 2, 4, 2, 1, bias=True),
#             nn.InstanceNorm2d(num_filters_last * 2, affine=True),
#             nn.LeakyReLU(0.2, inplace=True),

#             nn.Conv2d(num_filters_last * 2, num_filters_last * 4, 4, 2, 1, bias=True),
#             nn.InstanceNorm2d(num_filters_last * 4, affine=True),
#             nn.LeakyReLU(0.2, inplace=True),

#             nn.Conv2d(num_filters_last * 4, 1, 3, 1, 1, bias=True)
#         ]
...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_channels:int=3, num_filters_last=64, n_layers=3):
        ...
        layers = [
            ...
            # 第二层: 32x32 -> 16x16
            nn.Conv2d(num_filters_last, num_filters_last * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(num_filters_last * 2),
            nn.LeakyReLU(0.2),
            
            # 第三层: 16x16 -> 8x8
            nn.Conv2d(num_filters_last * 2, num_filters_last * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(num_filters_last * 4),
            nn.LeakyReLU(0.2),
            
            # 修改输出层: 保持 8x8
            nn.Conv2d(num_filters_last * 4, 1, 3, 1, 1)  # 改用 3x3 卷积核
        ]
